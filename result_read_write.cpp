...
void saveResultsDdm(AcqResults_t* acqResults, const Settings_t* settings)
{
    MATFile* pmat;
    //settings->fileName
    ...
    writeDoubleValueToMatFile(pmat, acqResults->codePhase,         32,               1, false, "codePhase");
    writeDoubleValueToMatFile(pmat, acqResults->peakMetric,        32,               1, false, "peakMetric");
    writeDoubleValueToMatFile(pmat, acqResults->frequencyBinIndex, 32,               1, false, "frequencyBinIndex");
    writeDoubleValueToMatFile(pmat, acqResults->DelayDopplerMap,   53000 * 101 * 32, 1, false, "DelayDopplerMap");

    // Save additional settings in MAT format
      // MATLAB settings
    acqResults->matSaveSettings.acquisitionType = (settings->acqMethod == NORMAL_SIGNAL_ACQUISITION) ? 0.0 : 1.0;
    acqResults->matSaveSettings.acqSearchBand = settings->acqSearchBand;
    acqResults->matSaveSettings.samplingFreq = settings->samplingFreq;
    acqResults->matSaveSettings.iF = settings->iF;
    acqResults->matSaveSettings.codeFreqBasis = settings->codeFreqBasis;
    acqResults->matSaveSettings.codeLength = settings->codeLength;

    writeDoubleValueToMatFile(pmat, &acqResults->matSaveSettings.acquisitionType, 1, 1, false, "acquisitionType");
    writeDoubleValueToMatFile(pmat, &acqResults->matSaveSettings.acqSearchBand,   1, 1, false, "acqSearchBand");
    writeDoubleValueToMatFile(pmat, &acqResults->matSaveSettings.samplingFreq,    1, 1, false, "samplingFreq");
    writeDoubleValueToMatFile(pmat, &acqResults->matSaveSettings.iF,              1, 1, false, "IF");
    writeDoubleValueToMatFile(pmat, &acqResults->matSaveSettings.codeFreqBasis,   1, 1, false, "codeFreqBasis");
    writeDoubleValueToMatFile(pmat, &acqResults->matSaveSettings.codeLength,      1, 1, false, "codeLength");
