...
typedef struct matlabSaveSettings_s
{
    double acquisitionType;
    double acqSearchBand;
    double PIT;
    double samplingFreq;
    double iF;
    double codeFreqBasis;
    double codeLength;
} matlabSaveSettings_t;

typedef struct {
  double carrFreq[32];
  double codePhase[32];
  double peakMetric[32];
  double frequencyBinIndex[32];
  double DelayDopplerMap[32*53000*101];
  matlabSaveSettings_t matSaveSettings;
} AcqResults_t;
