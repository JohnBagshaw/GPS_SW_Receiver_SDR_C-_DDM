...
void acquisitionCore(const Settings_t* settings,AcqResults_t* acqResults,
  int    PRNIdx,double* signal1,double* signal2,double* signal0DC,double** caCodeTables,bool* sigFound, bool ddmProcessingEn
)
{
  ...
  int PRN = (int)settings->acqSatelliteList[PRNIdx];
    ...
    if (ddmProcessingEn)
    {
        ...
        for (size_t i = 0; i < numberOfFrqBins; i++)
        {
            for (size_t j = 0; j < samplesPerCode; j++)
            {
                acqResults->DelayDopplerMap[PRNIdx * len + i * (size_t)samplesPerCode + j] = results[i][j];
            }
        }
    }
    ...
    if (ddmProcessingEn)
    {
        acqResults->frequencyBinIndex[PRNIdx] = frequencyBinIndex;
    }
...
bool acquisition(const double* longSignal,const Settings_t* settings,AcqResults_t* acqResults, bool ddmProcessingEn)
{
#define FUNC_PREFIX             "acquisition(): "
  ...
  for (size_t i = 0; i < totalThreads; i++)
  {
    for (size_t j = 0; j < numParallelThreads; j++)
    {
      ...
      parallelAcquisitionThreads[PRNIdx] = thread(acquisitionCore,settings,acqResults,PRNIdx,signal1,signal2,signal0DC,caCodeTables,&sigFoundAny[i], ddmProcessingEn);
    }
